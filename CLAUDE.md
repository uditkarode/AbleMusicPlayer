...
- **Build**: Gradle 8.7, AGP 8.5.2, Kotlin 2.0.20, version catalog (`gradle/libs.versions.toml`)
...
:core:services          Duplicate MusicService/DownloadService (unused, app module has its own)
:core:utils             Test stubs only
...
| `Home` (fragment) | Tab 1. Song list, `streamAudio()`, `cacheMusic()` (cache-while-stream pipeline) |
...
| `AbleApplication` | `Application` subclass, initializes `Constants.init(this)` |
...
- **`MusicMode`** — `"Download"` / `"Stream"` / `"Both"`
...
| OkHttp 4.12.0 | HTTP client for NewPipe downloader and streaming |
| Glide 4.14.2 | Image loading (album art, thumbnails) |
| Gson 2.10.1 | Playlist JSON serialization |
| Lottie 6.0.0 | Loading animations |
| Material Dialogs 3.3.0 | Dialogs and bottom sheets |
| Calligraphy3 + ViewPump | Custom font injection (Inter) |
| Palette KTX | Album art color extraction for player theming |
| jaudiotagger-android | Audio metadata tag reading/writing |

## Build & Run

Requires **JDK 17** (Gradle 8.7 / Kotlin 2.0.20 do not support JDK 25):
```bash
